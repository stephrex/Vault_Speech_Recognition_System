...
import matplotlib.pyplot as plt
import tf_keras as tf
# Only needed if you trained with it
from sklearn.preprocessing import StandardScaler
import io
...
def preprocess_audio_file(file_obj, target_sr=16000, n_mfcc=40, fixed_length=90, ndim=3):
    # Load audio directly from uploaded file (file-like object)
    x, sr = librosa.load(file_obj, sr=target_sr, mono=True)
    ...
    # Extract MFCCs
    mfccs = librosa.feature.mfcc(y=x, sr=sr, n_mfcc=n_mfcc)
    mfccs = np.moveaxis(mfccs, 1, 0)  # shape = (time, n_mfcc)
    ...
    # (Optional) Normalize if model was trained with normalized features
    # scaler = joblib.load("scaler.pkl")
    # mfccs = scaler.transform(mfccs.reshape(-1, mfccs.shape[-1])).reshape(1, fixed_length, n_mfcc, 1)
...
if uploaded_file is not None:
    ...
    # Load waveform for display (not prediction)
    x_disp, sr_disp = librosa.load(uploaded_file, sr=None)
    st.write("Waveform:")
    fig, ax = plt.subplots()
    ...
    try:
        ...
        processed_audio = preprocess_audio_file(uploaded_file)
        prediction_prob = model.predict(processed_audio)
        st.write(prediction_prob)
        prediction_prob = np.squeeze(prediction_prob)
        st.write(prediction_prob)
